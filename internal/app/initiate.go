...
func (a *App) initInstrument() {
	ins, err := instrument.New(context.Background(), &instrument.Config{
		...
		MetricsIntervalSeconds: a.config.GetSecond("instrument.metric_interval_seconds"),
		MaskFields:             a.config.GetArray("instrument.log.mask_fields"),
	})
	if err != nil {
...
`
	...
	adapter, err := pgxcasbin.NewAdapter(a.ctx, a.dbConn, pgxcasbin.WithTableName("auth_casbin_rules"))
	if err != nil {
	...
	watcher, err := pgxcasbin.NewWatcherWithPool(a.ctx, a.dbConn,
		pgxcasbin.OptionWatcher{
			NotifySelf: true,
			Channel:    "auth_casbin_psql_watcher",
			Verbose:    false,
	...
	if err := watcher.SetUpdateCallback(pgxcasbin.DefaultCallback(enforcer)); err != nil {
...
func (a *App) initHTTPServer() {
	...
	corsHandler := cors.New(cors.Options{
		AllowedOrigins: []string{a.config.GetString("app.web")},
