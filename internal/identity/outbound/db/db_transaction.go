...
func (s *DB) ReplaceChallenge(ctx context.Context, chal entity.Challenge) (err error) {
	ctx, span := s.startSpan(ctx, "ReplaceChallenge")
	defer func() { s.endSpan(span, err) }()
	...
	if err := wtx.DeleteIdentityChallengesByUserPurpose(ctx, sqlc.DeleteIdentityChallengesByUserPurposeParams{
		UserID:  chal.UserID,
		Purpose: chal.Purpose,
	}); err != nil {
		return s.mapError(err)
