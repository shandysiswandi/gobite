...
type repoDB interface {
	...
	NewMFAFactorTOTPBackup(ctx context.Context, fTOTP, fBackup entity.MFAFactor, codes []entity.MFABackupCode, challengeID int64) error
	NewRefreshToken(ctx context.Context, ref entity.RefreshToken, challengeID int64) error
	NewRegistration(ctx context.Context, user entity.User, chal entity.Challenge, hash string) error
	NewBackupCodes(ctx context.Context, codes []entity.MFABackupCode) error
...
func (s *Usecase) isAuthorized(ctx context.Context, sub, obj, act string) error {
	ok, err := s.enforcer.Enforce(sub, obj, act)
	if err != nil {
		slog.ErrorContext(ctx, "failed to check authorization", "user_id", sub, "error", err)
