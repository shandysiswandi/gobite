...
func (s *Usecase) verifyTurnstile(ctx context.Context, token string) error {
	ok, err := s.turnstile.Verify(ctx, token)
	if err != nil {
		slog.ErrorContext(ctx, "failed to verify turnstile captcha", "error", err)
		return goerror.NewServer(err)
	}

	if !ok {
		slog.WarnContext(ctx, "verify turnstile captcha not match")
		return goerror.NewInvalidFormat("Incorrect Turnstile response. Try the verification again")
	}
