...
func (s *DB) UpsertUserSetting(ctx context.Context, userID int64, setting entity.UserSetting) (err error) {
	ctx, span := s.startSpan(ctx, "UpsertUserSetting")
	defer func() { s.endSpan(span, err) }()

	err = s.query.UpsertNotificationUserSetting(ctx, sqlc.UpsertNotificationUserSettingParams{
		UserID:     userID,
