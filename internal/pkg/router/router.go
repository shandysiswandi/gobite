...
// @Tags System
// @Produce json
// @Success 200 {object} messageResponse "Root response"
// @Router /health [get]
func healthHandler(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, messageResponse{Message: "server is running well"}, http.StatusOK)
}
...
// @Tags System
// @Produce json
// @Success 200 {object} messageResponse "Health response"
// @Router / [get]
func rootHandler(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, messageResponse{Message: "hi from gobite"}, http.StatusOK)
}

// NewRouter builds the default application router with standard middleware.
func NewRouter(cfg config.Config, uuid uid.StringID, verifier jwt.JWT) *Router {
	hr := &httprouter.Router{
	...
	ro := &Router{
		...
		mws: []Middleware{
			...
			middlewareLogging,
			middlewareIP,
			middlewareAuthentication(cfg, verifier),
		},
	}

	hr.Handler(http.MethodGet, "/", http.HandlerFunc(rootHandler))
	hr.Handler(http.MethodGet, "/health", http.HandlerFunc(healthHandler))
