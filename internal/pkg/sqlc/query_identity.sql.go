...
const deleteIdentityChallengesByUserPurpose = `-- name: DeleteIdentityChallengesByUserPurpose :exec
DELETE FROM identity_challenges WHERE user_id = $1 AND purpose = $2
`

type DeleteIdentityChallengesByUserPurposeParams struct {
	UserID  int64
	Purpose identity_entity.ChallengePurpose
}

func (q *Queries) DeleteIdentityChallengesByUserPurpose(ctx context.Context, arg DeleteIdentityChallengesByUserPurposeParams) error {
	_, err := q.db.Exec(ctx, deleteIdentityChallengesByUserPurpose, arg.UserID, arg.Purpose)
	return err
}
