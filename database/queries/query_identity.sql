...
FROM identity_users
WHERE
    (NOT @filter_by_status::boolean OR status = @status)
    AND (
...
ORDER BY
  -- email
  CASE WHEN @order_by::text = 'email:asc'  THEN email END ASC,
  CASE WHEN @order_by::text = 'email:desc' THEN email END DESC,
  -- full_name
  CASE WHEN @order_by::text = 'full_name:asc'  THEN full_name END ASC,
  CASE WHEN @order_by::text = 'full_name:desc' THEN full_name END DESC,
  -- updated_at
  CASE WHEN @order_by::text = 'updated_at:asc'  THEN updated_at END ASC,
  CASE WHEN @order_by::text = 'updated_at:desc' THEN updated_at END DESC,
  -- status
  CASE WHEN @order_by::text = 'status:asc'  THEN status END ASC,
  CASE WHEN @order_by::text = 'status:desc' THEN status END DESC,
  -- fallback
